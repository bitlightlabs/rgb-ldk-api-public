...
3) View the generated OpenAPI doc (Redoc):

- open `docs/openapi.html` (it reads `generated/spec/openapi.json`)

Notes:

- `generated/` is overwritten by the sync step.
...
- `python3 scripts/rgb-ldk-api.py gen` also sanitizes `generated/spec/source.json` to avoid committing local paths (you can run `python3 scripts/rgb-ldk-api.py sanitize` standalone).
